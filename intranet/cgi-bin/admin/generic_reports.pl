...
#
#
if (!$tablename){ #No hay nada seleccionado
$tablename=' ';
push (@tablenames,' ');
}
#
...
$template->param(fieldnames=> $fieldnames);
		}
...
$template->param(tables1 => $tables1);
...
$template->param(fields1=> $fields1);
		}
...
$template->param(tables2 => $tables2);
...
$template->param(fields2=> $fields2);
...
$template->param(jointype=> $jointype);
