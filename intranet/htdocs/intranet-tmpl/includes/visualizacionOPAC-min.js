var objAH;CAMPOS_ARRAY=new Array();SUBCAMPOS_ARRAY=new Array();function eliminarVista(vista_id){objAH=new AjaxHelper(updateAgregarVisualizacion);objAH.debug=true;objAH.showOverlay=true;objAH.url="/cgi-bin/koha/catalogacion/visualizacionOPAC/visualizacionOpacDB.pl";objAH.tipoAccion='ELIMINAR_VISUALIZACION';if(vista_id){jConfirm(ESTA_SEGURO_QUE_DESEA_BORRARLO,CATALOGO_ALERT_TITLE,function(confirmStatus){if(confirmStatus){objAH.vista_id=vista_id;objAH.sendToServer();}});}}
function agregarVisualizacion(){objAH=new AjaxHelper(updateAgregarVisualizacion);objAH.debug=true;objAH.showOverlay=true;objAH.url="/cgi-bin/koha/catalogacion/visualizacionOPAC/visualizacionOpacDB.pl";objAH.tipoAccion='AGREGAR_VISUALIZACION';var perfil=$("#perfiles_ref").val();var campo=$.trim($("#campo").val());var subcampo=$.trim($("#subcampo").val());var liblibrarian=$.trim($("#liblibrarian").val());if((perfil)&&(campo)&&(subcampo)&&(liblibrarian)){objAH.perfil=perfil;objAH.campo=campo;objAH.subcampo=subcampo;objAH.liblibrarian=liblibrarian;objAH.sendToServer();}else{jAlert(SELECCIONE_VISTA_OPAC,CATALOGO_ALERT_TITLE);}}
function updateAgregarVisualizacion(responseText){eleccionDePerfil();}
function eleccionDePerfil(){var perfil=$("#perfiles_ref").val();var ObjDiv=$("#result");if(isNaN(perfil)){ObjDiv.hide();}else{ObjDiv.show();objAH=new AjaxHelper(updateEleccionDeNivel);objAH.debug=true;objAH.showOverlay=true;objAH.url="/cgi-bin/koha/catalogacion/visualizacionOPAC/visualizacionOpacDB.pl";objAH.tipoAccion='MOSTRAR_VISUALIZACION';objAH.perfil=perfil;objAH.sendToServer();}}
function updateEleccionDeNivel(responseText){$("#result").html(responseText);zebra("tabla_datos");}
function eleccionCampoX(){if($("#campoX").val()!=-1){objAH=new AjaxHelper(updateEleccionCampoX);objAH.debug=true;objAH.showOverlay=true;objAH.url="/cgi-bin/koha/catalogacion/visualizacionOPAC/visualizacionOpacDB.pl";objAH.campoX=$('#campoX').val();objAH.tipoAccion="GENERAR_ARREGLO_CAMPOS";objAH.sendToServer();}
else
...
function eleccionCampo(){if($("#campo").val()!=-1){objAH=new AjaxHelper(updateEleccionCampo);objAH.debug=true;objAH.showOverlay=true;objAH.url="/cgi-bin/koha/catalogacion/visualizacionOPAC/visualizacionOpacDB.pl";objAH.campo=$('#campo').val();objAH.tipoAccion="GENERAR_ARREGLO_SUBCAMPOS";objAH.sendToServer();}
else
enable_disableSelects();}
...
enable_disableSelects();}
function mostrarTablaRef(){objAH=new AjaxHelper(updateMostrarTablaRef);objAH.debug=true;objAH.showOverlay=true;objAH.url="/cgi-bin/koha/utils/utilsDB.pl";objAH.tipoAccion="GENERAR_ARREGLO_TABLA_REF";objAH.sendToServer();}