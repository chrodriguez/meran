...
sub cardGenerator {
	...
	my $dbh = C4::Context->dbh;
        my $sth=$dbh->prepare("SELECT borrowernumber, documenttype, documentnumber, studentnumber, firstname, surname FROM borrowers WHERE borrowernumber=$bornum");
	$sth->execute;
	my ($borrowernumber, $documenttype, $documentnumber, $studentnumber, $firstname, $surname) = $sth->fetchrow_array;
	$sth->finish;
	...
	$pdf->addRawText("Apellido: $surname",10,$pageheight - 15);
	$pdf->addRawText("Nombre: $firstname",10,$pageheight - 30);
	$pdf->addRawText("$documenttype: $documentnumber",10,$pageheight - 45);
	$pdf->addRawText("Legajo: $studentnumber",10,$pageheight - 60);
