var objAH;CAMPOS_ARRAY=new Array();SUBCAMPOS_ARRAY=new Array();function mostrarTabla(){objAH=new AjaxHelper(updateMostrarTabla);objAH.debug=true;objAH.showOverlay=true;objAH.url=URL_PREFIX+"/catalogacion/visualizacionOPAC/visualizacionOpacDB.pl";objAH.tipoAccion='MOSTRAR_TABLA_CAMPO';objAH.ejemplar=$("#tipo_nivel3_id").val();objAH.nivel=$("#eleccion_nivel").val();objAH.sendToServer();}
...
function updateSortable(){mostrarTabla();}
function eliminarTodoElCampo(campo){objAH=new AjaxHelper(updateEliminarTodoCampo);objAH.debug=true;objAH.showOverlay=true;objAH.url=URL_PREFIX+"/catalogacion/visualizacionOPAC/visualizacionOpacDB.pl";objAH.tipoAccion='ELIMINAR_TODO_EL_CAMPO';objAH.nivel=$("#eleccion_nivel").val();objAH.ejemplar=$("#tipo_nivel3_id").val();if(campo){jConfirm(SEGURO_QUE_DESEA_ELIMINAR_TODO_EL_CAMPO,CATALOGO_ALERT_TITLE,function(confirmStatus){if(confirmStatus){objAH.campo=campo;objAH.sendToServer();}});}}
...
function eliminarVista(vista_id){objAH=new AjaxHelper(updateAgregarVisualizacion);objAH.debug=true;objAH.showOverlay=true;objAH.url=URL_PREFIX+"/catalogacion/visualizacionOPAC/visualizacionOpacDB.pl";objAH.tipoAccion='ELIMINAR_VISUALIZACION';if(vista_id){jConfirm(ESTA_SEGURO_QUE_DESEA_BORRARLO,CATALOGO_ALERT_TITLE,function(confirmStatus){if(confirmStatus){objAH.vista_id=vista_id;objAH.sendToServer();}});}}
...
function eleccionDeEjemplar(){var ejemplar=$("#tipo_nivel3_id").val();var ObjDiv=$("#result");if(!isNaN(ejemplar)){ObjDiv.hide();}else{ObjDiv.show();objAH=new AjaxHelper(updateEleccionDeNivel);objAH.debug=true;objAH.showOverlay=true;objAH.url=URL_PREFIX+"/catalogacion/visualizacionOPAC/visualizacionOpacDB.pl";objAH.tipoAccion='MOSTRAR_VISUALIZACION';objAH.ejemplar=ejemplar;objAH.sendToServer();}}
