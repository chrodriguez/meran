...
__PACKAGE__->meta->setup(
    ...
    columns => [
        ...
        campoIso         => { type => 'integer', overflow => 'truncate', default => '0', not_null => 1 },
        subCampoIso      => { type => 'character', overflow => 'truncate', length => 1 },
        ...
        ui               => { type => 'text', overflow => 'truncate', length => 65535, not_null => 1 },
        marc_koha_field  => { type => 'text', overflow => 'truncate', length => 65535, not_null => 1 },
        marc_table_field => { type => 'text', overflow => 'truncate', length => 65535, not_null => 1 },
        orden            => { type => 'integer', overflow => 'truncate' },
