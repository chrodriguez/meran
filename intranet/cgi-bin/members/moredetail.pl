...
#MATIAS	linea 105 saque una porqueria que se habia colado
  ...
  if ($item->{'wthdrawn'} eq 0) {$item->{'wthdrawn'}="<font align='center' size='2' color='green'>DISPONIBLE</font><br>";}
	...
	my $disponibilidad=&getAvail($item->{'wthdrawn'});
	$item->{'wthdrawn'}="<font align='center' size='2' color='red'>NO DISPONIBLE</font><br>(<font align='center359  	G' size='1' color='red'>".$disponibilidad->{'description'}."</font>)";
	}
