...
#Agregado por Einar, para obtener el listado de ciudades.

sub getcity {
# returns a reference to a hash of references to branches...
	my $loc=shift;
	my $dbh = C4::Context->dbh;
	my $sth=$dbh->prepare("select * from localidades where LOCALIDAD = ?");
	$sth->execute($loc);
	my $city=$sth->fetchrow_hashref;
	return ($city->{'NOMBRE'});
}
...
# returns a reference to a hash of references to branches...
	my %cities;
	my $dbh = C4::Context->dbh;
	my $sth=$dbh->prepare("select * from localidades");
	$sth->execute;
	while (my $city=$sth->fetchrow_hashref) {
			$cities{$city->{'LOCALIDAD'}}=$city;}
	return (\%cities);
}
...
my $query="update borrowers set 
		...
		faxnumber='$update->{'faxnumber'}',
		firstname='$update->{'firstname'}',
  		emailaddress='$update->{'emailaddress'}',
  		city='$update->{'city'}',
		phone='$update->{'phone'}',
  		surname='$update->{'surname'}'
  		where borrowernumber='$update->{'borrowernumber'}'";
