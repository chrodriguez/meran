	...
	unless ($userid) { #si no hay userid, hay que autentificarlo y no existe sesion
		...
		if ($flags = haspermission($dbh, $userid, $flagsrequired)) {
			...
			if ($type eq 'opac') {
				...
				my ($isSanction,$endDate)= C4::AR::Sanctions::permitionToLoan($dbh, getborrowernumber($userid), C4::Context->preference("defaultissuetype"));
				my $regular= C4::AR::Reserves::isRegular(getborrowernumber($userid));
				
				if ($isSanction || !$regular ){
				C4::AR::Reserves::cancelar_reservas(getborrowernumber($userid));
								}
			} else {
				#Si es un usuario de intranet entonces se borran las reservas de todos los usuarios sancionados
				C4::AR::Reserves::cancelar_reservas(C4::AR::Sanctions::getBorrowersSanctions($dbh, C4::Context->preference("defaultissuetype")));
				#Ademas, se borran las reservas de los usuarios que no son alumnos regulares
				C4::AR::Reserves::cancelar_reservas(C4::AR::Reserves::FindNotRegularUsersWithReserves());
				C4::AR::Reserves::eliminarReservasVencidas();
