...
my ($template, $personnumber, $cookie)
    = get_template_and_user({template_name => "members/newmember2.tmpl",
			     query => $input,
                             type => "intranet",
                             authnotrequired => 0,
...
if ($data{'cardnumber'} eq ''){
    ...
    my $nounique;
    if ( $data{'type'} eq "Add" )    {
	        my $opcion= checkDocument($data{'documenttype'},$data{'documentnumber'},"borrowers");
                if ($opcion) {
