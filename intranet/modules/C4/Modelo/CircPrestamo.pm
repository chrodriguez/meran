...
#**********************************Se registra el movimiento en rep_historial_circulacion***************************
	...
	C4::AR::Debug::debug( "CircPrestamo => agregar => responsable: "
		  . $data_hash->{'responsable'} );
	use C4::Modelo::RepHistorialCirculacion;
	my ($historial_circulacion) =
	  C4::Modelo::RepHistorialCirculacion->new( db => $self->db );
...
sub prestar {
	...
	C4::AR::Debug::debug(
		"_chequeoParaPrestamo=> tipo_prestamo: " . $params->{'tipo_prestamo'} );
...
# Tener en cuenta los prestamos especiales, $tipo_prestamo ==> ES ---> SA. **** VER!!!!!!
	...
	if ( $cant == 1 && $disponibilidad eq "Domiciliario" ) {
		...
		$self->debug( "El usuario ya tiene una reserva ID::: "
			  . $reservas->[0]->getId_reserva );
...
#     $reserva->load();
	...
	my ($reserva) =
	  C4::AR::Reservas::getReserva( $params->{'id_reserva'}, $self->db );
	$self->debug(
		"Se actualiza el estado de la reserva a P = Prestado a la reserva "
		  . $reserva->getId2 );
...
# Se borra la sancion correspondiente a la reserva porque se esta prestando el biblo
	...
	C4::AR::Debug::debug( "CircPrestamo => insertarPrestamo => responsable "
		  . $params->{'responsable'} );
...
"Se verifica si el usuario llego al maximo de prestamos, se caen las demas reservas"
		...
		$msg_object->{'error'} = 0;
		C4::AR::Mensajes::add( $msg_object,
			{ 'codMsg' => 'P108', 'params' => [ $params->{'barcode'} ] } );
		my ($reserva) = C4::Modelo::CircReserva->new( db => $self->db );
...
sub estaVencido {
	...
	my $dateformat = C4::Date::get_date_format();
	my $hoy =
	  C4::Date::format_date_in_iso( C4::Date::ParseDate("today"), $dateformat );
...
sub getFecha_vencimiento {
	...
	{ #es la primer renovacion por lo tanto tengo que ver sobre los dias de un prestamo normal para saber si estoy en fecha de renovacion
		...
		return (
			C4::Date::proximoHabil(
				$plazo_actual, 0, $self->getFecha_prestamo
			)
		);
	}
}
...
sub devolver {
	...
	my $fechaVencimiento = $self->getFecha_vencimiento
	  ;    # tiene que estar aca porque despues ya se marco como devuelto
	       #Actualizo la fecha de devolucion!!!!
	my $dateformat = C4::Date::get_date_format();
	my $fechaHoy =
	  C4::Date::format_date_in_iso( ParseDate("today"), $dateformat );
...
#**********************************Se registra el movimiento en rep_historial_circulacion***************************
		...
		C4::AR::Debug::debug(
			"CircPrestamo=> devolver => responsable" . $loggedinuser );
...
# Hay que ver si devolvio el ejemplar a termino para, en caso contrario, aplicarle una sancion
		...
		use C4::AR::Sanciones;
		my $diasSancion =
		  C4::AR::Sanciones::diasDeSancion( $fechaHoy, $fechaVencimiento,
...
# Se calcula la fecha de fin de la sancion en funcion de la fecha actual (hoy + cantidad de dias de sancion)
				$fechaFinSancion = C4::Date::format_date_in_iso(
					DateCalc(
						ParseDate("today"), "+ " . $diasSancion . " days",
						\$err
					),
					$dateformat
				);
...
#**********************************Se registra el movimiento en rep_historial_prestamo***************************
		...
		my $historial_prestamo =
		  C4::Modelo::RepHistorialPrestamo->new( db => $self->db );
...
sub estaEnFechaDeRenovacion {
	...
	my $hoy =
	  C4::Date::format_date_in_iso(
		DateCalc( ParseDate("today"), "+ 0 days", \$err ), $dateformat );
	my $desde = C4::Date::format_date_in_iso(
		DateCalc(
			$self->getFecha_vencimiento,
			"- " . $self->tipo->getDias_antes_renovacion . " days",
			\$err, 2
		),
		$dateformat
	);
...
#**********************************Se registra el movimiento en rep_historial_circulacion***************************
	...
	$data_hash->{'responsable'} = $loggedinuser;
	C4::AR::Debug::debug(
		"CircPrestamo=> renovar => responsable" . $loggedinuser );
...
sub getInvolvedCount {
	...
	my $circ_prestamo_count =
	  C4::Modelo::CircPrestamo::Manager->get_circ_prestamo_count(
		query => \@filtros );
