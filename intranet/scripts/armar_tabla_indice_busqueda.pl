...
while ( my $id1 = $sth->fetchrow ) {
	...
	while ( my $id2 = $sth2->fetchrow ) {
		...
		if ( $nivel2_object ne 0 ) {
			...
			C4::AR::Debug::debug(
				'MARCDetail => cant ' . scalar(@$marc_array_nivel2) );
	...
	for ( my $i = 0 ; $i < scalar(@result) ; $i++ ) {
		my $field = MARC::Field->new( @result[$i]->{'campo'},
			'', '', @result[$i]->{'subcampo'} => @result[$i]->{'dato'} );
	...
	my $query4 =
"INSERT INTO indice_busqueda (id,titulo,autor,string,registro_marc) VALUES (?,?,?,?,?) ";
