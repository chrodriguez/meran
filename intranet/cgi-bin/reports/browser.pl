...
my ($template, $loggedinuser, $cookie)
    = get_template_and_user({template_name => "reports/browser.tmpl",
			     query => $input,
			     type => "intranet",
			     authnotrequired => 0,
...
my @resultsdata= biblioitems($orden,$ini,$cantR);#Biblioitems
my $cantidad=cantidadBiblio($branch);#Cantidad total de biblioitems  para calcular la cantidad de paginas
