...
__PACKAGE__->meta->setup(
    ...
    columns => [
        ...
        codigo          => { type => 'character',length => 2 ,not_null => 1 },
        descripcion     => { type => 'varchar', length => 30, not_null => 1 },
    ],
...
# 	return ($ref_valores->[0]->getCampo($campo));
  if(scalar(@$ref_valores) > 0){
    ...
    C4::AR::Debug::debug("RefColaborador => obtenerValorCampo => no se pudo recuperar el objeto");
    return 'NO TIENE';
  }
}
