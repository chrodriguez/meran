...
# $Id: Circulation.pm,v 1.9 2003/12/03 11:43:12 slef Exp $
...
NULL");
  ...
  if ($borrower->{'borrowernumber'} ne ''){
    if ($bornum eq $borrower->{'borrowernumber'}){
      ...
      my $resp = &msg_yni("El libro est&aacute; prestado a este usuario","Renovar?");
      if ($resp == "y") {
      ...
      my $resp = &msg_yn($text,"Marcar como devuelto?");
      if ($resp == "y") {
