...
$search{'title'}=$title;
my $keyword=$input->param('id'); #estaba d, pero creo que es id
...
while ( my ($key, $value) = each %search) {
	if ($value ne ''){
		...
		if ($key eq 'title'){$linesearch{key}='T&iacute;tulo';}
		elsif ($key eq 'author'){$linesearch{key}='Autor';}
		elsif ($key eq 'key'){$linesearch{key}='Palabra Clave';}
		else {$linesearch{key}=$key;}
