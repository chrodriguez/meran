...
my $branch= getbranch($query, $branches);#14/03/07-No se veia la biblioteca-Damian  
#my $branch= (split("_",(split(";",$cookie))[0]))[1]; Esto era lo que estaba antes
...
if ($barcode) {
	...
	if ($transfered) {
		...
		if (not ($found)) {
			($clase eq $linecolor1) ? ($clase=$linecolor2) : ($clase=$linecolor1);
			$item{'clase'}=$clase;
