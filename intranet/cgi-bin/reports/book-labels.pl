...
$params{'id_ui'} = $ui;
$params{'minBarcode'} = $barcode1;
$params{'maxBarcode'} = $barcode2;
$params{'orden'} = $orden;
$params{'beginLocation'} = $bulkbegin;
$params{'minLocation'} = $bulk1;
$params{'maxLocation'} = $bulk2;
C4::AR::Utilidades::printHASH(\%params);
# FIXME VER COMO CIERRAN LOS IF... NO ME GUSTA COMO QUEDO
    my ($cantidad,$results)=  C4::AR::Estadisticas::listaDeEjemplares(\%params);
# if ($op eq 'pdf') {
# #HAY QUE GENERAR EL PDF CON LOS CARNETS
#     ($cantidad,$results)=  C4::AR::Estadisticas::listaDeEjemplares(\%params);
