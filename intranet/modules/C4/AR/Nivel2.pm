...
sub t_guardarNivel2 {
	...
	if ( !$msg_object->{'error'} ) {
		...
		( $msg_object, $id2 ) =
		  guardarRealmente( $msg_object, $params->{'id1'}, $marc_record );
	}
...
sub t_eliminarNivel2 {
	...
	if ( !$msg_object->{'error'} ) {
		...
		eval {
			...
			$db->commit;
			C4::AR::Busquedas::generar_indice(
				$cat_registro_marc_n2->getId1() );
			C4::AR::Busquedas::reindexar();

			#se cambio el permiso con exito
			$msg_object->{'error'} = 0;
			C4::AR::Mensajes::add( $msg_object,
				{ 'codMsg' => 'U375', 'params' => [$id2] } );
		};
...
sub _verificarDeleteNivel2 {
	...
	if ( !( $msg_object->{'error'} )
		&& $cat_registro_marc_n2->tienePrestamos() )
	{
		C4::AR::Debug::debug(
			"_verificarDeleteNivel2 => tiene al menos 1 ejemplar prestado ");

   #verifico que el nivel2 que quiero eliminar no tenga ningun ejemplar prestado
		$msg_object->{'error'} = 1;
		C4::AR::Mensajes::add( $msg_object,
			{ 'codMsg' => 'P124', 'params' => [ $params->{'id2'} ] } );

	}
	elsif ( !( $msg_object->{'error'} )
		&& $cat_registro_marc_n2->tieneReservas() )
	{

  #verifico que el nivel2 que quiero eliminar no tenga ningun ejemplar reservado
		$msg_object->{'error'} = 1;
		C4::AR::Debug::debug(
"_verificarDeleteNivel2 => Se está intentando eliminar un ejemplar que tiene al menos un ejemplar reservado "
		);
		C4::AR::Mensajes::add( $msg_object,
			{ 'codMsg' => 'P123', 'params' => [ $params->{'id2'} ] } );
	}
...
# 2) se podria usar el indice??????????????
	...
	for ( my $i = 0 ; $i < $cant ; $i++ ) {
		...
		if ( $cat_registro_marc_n2_array_ref->[$i]->getTipoDocumentoObject() eq
			$tipo_doc )
		{
			push( @cat_registro_marc_n2_array_ref_result,
				$cat_registro_marc_n2_array_ref->[$i] );
			last();
		}
	}
...
## FIXME ver si falta verificar algo!!!!!!!!!!
	...
	if ( !$msg_object->{'error'} ) {
		...
		eval {
			my $marc_record =
			  C4::AR::Catalogacion::meran_nivel2_to_meran($params);
			...
			C4::AR::Mensajes::add(
				...
				{
					'codMsg' => 'U381',
					'params' => [ $cat_registro_marc_n2->getId2 ]
				}
			);
		};
...
sub getRating {
	...
	my $rating =
	  C4::Modelo::CatRating::Manager->get_cat_rating( query => \@filtros, );
	my $rating_count =
	  C4::Modelo::CatRating::Manager->get_cat_rating_count( query => \@filtros,
	  );
	my $count = 0;
...
sub getReviews {
	...
	push( @filtros, ( id2    => { eq => $id2 } ) );
	push( @filtros, ( review => { ne => NULL } ) );
	my $reviews = C4::Modelo::CatRating::Manager->get_cat_rating(
		query           => \@filtros,
