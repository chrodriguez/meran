...
# $Id: fines2.pl,v 1.9.2.1 2003/12/18 00:04:13 slef Exp $
use HTML::Template;
use strict;
require Exporter;
use C4::Context;
...
use C4::Circulation::FinesPrueba;
use Date::Manip;
...
# Yep
my $maxFine=5;
#supone un maximo de suspesion de 5
# FIXME
...
#en date la fecha de Hoy, y en date2 la fecha del prestamo vencido
for (my $i=0;$i<$numOverdueItems;$i++){
  ...
  if ($date2 <= $date){#nunca va a dar igual ya que en el Sql pregunto solo las menores
...
#      print "$borrower->{'cardnumber'}\t$borrower->{'categorycode'}\t0 fine\n";
	...
	if ($item->{'itemlost'} ne '1' && $item->{'itemlost'} ne '2' ){
	  ...
	  $accountno,$cost,"Ejemplar perdido $item->{'title'} $item->{'barcode'} $due",$cost);
	  $sth->finish;
