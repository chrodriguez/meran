...
function buscarPorTema(){objAH=new AjaxHelper(updateInfoBusquedas);objAH.debug=true;objAH.showOverlay=true;objAH.url=URL_PREFIX+'/busquedas/busquedasDB.pl';objAH.tema=$('#tema').val();objAH.tipoAccion='BUSQUEDA_POR_TEMA';objAH.funcion='changePage';objAH.sendToServer();}
function buscarPorISBN(){objAH=new AjaxHelper(updateInfoBusquedas);objAH.showOverlay=true;objAH.debug=true;objAH.url=URL_PREFIX+'/busquedas/busquedasDB.pl';objAH.isbn=$('#isbn').val();objAH.tipoAccion='BUSQUEDA_POR_ISBN';objAH.only_available=($('#only_available').attr('checked'))?1:0;objAH.sendToServer();}
function buscarPorDiccionario(){objAH=new AjaxHelper(updateInfoBusquedas);objAH.debug=true;objAH.showOverlay=true;objAH.url=URL_PREFIX+'/busquedas/diccionario.pl';objAH.dictionary=$('#dictionary').val();objAH.only_available=($('#only_available').attr('checked'))?1:0;objAH.funcion='changePage';objAH.sendToServer();}
function buscarPorCodigoBarra(){objAH=new AjaxHelper(updateInfoBusquedas);objAH.debug=true;objAH.showOverlay=true;objAH.url=URL_PREFIX+'/busquedas/busquedasDB.pl';objAH.codBarra=$('#codBarra').val();objAH.only_available=($('#only_available').attr('checked'))?1:0;objAH.tipoAccion='BUSQUEDA_POR_BARCODE';objAH.sendToServer();}
...
function buscarEstante(){objAH=new AjaxHelper(updateInfoBusquedas);objAH.showOverlay=true;objAH.debug=true;objAH.url=URL_PREFIX+'/busquedas/estante.pl';objAH.viewShelfName=$('#estante').val();objAH.orden='title';objAH.only_available=($('#only_available').attr('checked'))?1:0;objAH.funcion='changePage';objAH.sendToServer();}
