...
function realizarImportacion(){$('#formImportarVisualizacion').submit();}
...
function eleccionDeEjemplar(){var ejemplar=$("#tipo_nivel3_id").val();var ObjDiv=$("#result");if(!isNaN(ejemplar)){ObjDiv.hide();}else{if($("#tipo_nivel3_id").val()=="ANA"){$("#eleccion_nivel option[value='3']").remove();}else{var result=$('#eleccion_nivel').find('option[value="3"]');if(result.length==0){$("#eleccion_nivel").append('<option value="3">Nivel 3</option>');}}
ObjDiv.show();objAH=new AjaxHelper(updateEleccionDeNivel);objAH.debug=true;objAH.showOverlay=true;objAH.url=URL_PREFIX+"/catalogacion/visualizacionOPAC/visualizacionOpacDB.pl";objAH.tipoAccion='MOSTRAR_VISUALIZACION';objAH.ejemplar=ejemplar;objAH.sendToServer();}}
