...
__PACKAGE__->meta->setup(
    ...
    columns => [
        ...
        dato      => { type => 'varchar', length => 500, not_null => 1 },
...
sub load{
    ...
    eval {
         ...
         unless( $self->SUPER::load(speculative => 1) ){
                 C4::AR::Debug::debug("CatNivel1Repetible=>  dentro del unless, no existe el objeto SUPER load");
                $error = 1;
         }

        C4::AR::Debug::debug("CatNivel1Repetible=>  SUPER load");
        return $self->SUPER::load(@_);
    };

    if($@){
        C4::AR::Debug::debug("CatNivel1Repetible=>  no existe el objeto");
        $error = 1;
    }
