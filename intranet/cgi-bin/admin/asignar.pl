...
my $asignando=$input->param('asignandoID');
my $borrar=$input->param('editandoBORRAR');
my $aux=$codigo;
my ($asignadoMSG,$borrandoMSG)=asignar($tabla,$camporeferencia,$codigo,$asignando,$borrar);
if ($borrandoMSG) {
$codigo=$asignando;
}
...
$template->param(camposloop => \@campos,
		...
		editandoorden=>$orden,
		asignandoID=>$asignando,
		asignandoMensaje=> $asignadoMSG,
		borrandoMensaje=> $borrandoMSG,
		editandoidborrado=> $aux,
		);
