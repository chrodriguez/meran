...
################## DEFAULT ##################################
	...
	my $env;
	if  ($searchfield ne '') {
		$template->param(searchfield => "<p>Ha realizado una b&uacute;squeda por <strong>$searchfield</strong></p>");
	}
	my ($count,$results)=StringSearch($env,$searchfield,'web');
	my $toggle='par';
	my @loop_data = ();
	############ Commented by Luciano to avoid paging because it dosn't work ############
