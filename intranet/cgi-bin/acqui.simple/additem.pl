...
# $Id: additem.pl,v 1.20.2.1 2004/01/12 16:37:12 tipaul Exp $
...
# check for item barcode # being unique
	...
	my $exists = itemdata($oldbibid->{'items'}->{'barcode'});
	push @errors,"barcode_not_unique" if($exists);
# MARC::Record builded => now, record in DB
	# if barcode exists, don't create, but report The problem.
	my $oldbiblionumber= NEWnewitem($dbh,$oldbibid->{'items'},$record,$bibid) unless ($exists);
	if ($exists) {
		$nextop = "additem";
		$itemrecord = $record;
	} else {
...
# loop through each subfield
	for my $i (0..$#subf) {
  next if ($tagslib->{$field->tag()}->{$subf[$i][0]}->{tab}  ne 10 && ($field->tag() ne $itemtagfield && $subf[$i][0] ne $itemtagsubfield));	$witness{$subf[$i][0]} = $tagslib->{$field->tag()}->{$subf[$i][0]}->{lib} if ($tagslib->{$field->tag()}->{$subf[$i][0]}->{tab}  eq 10);
  $this_row{$subf[$i][0]} =$subf[$i][1] if ($tagslib->{$field->tag()}->{$subf[$i][0]}->{tab}  eq 10);
	printf L "dato ".$subf[$i][0]." = ".$subf[$i][1]."\n";	
	if ($field->tag() eq $itemtagfield && $subf[$i][0] eq $itemtagsubfield){
		 $this_row{itemnum} = $subf[$i][1]; 
		 push @itemnums,$this_row{$subf[$i][0]} =$subf[$i][1]; #Se agrega un nuevo itemnumber
		}
	}
...
# loop through each subfield
	foreach my $subfield (sort keys %{$tagslib->{$tag}}) {
		...
		$subfield_data{tag_lib}=$tagslib->{$tag}->{lib};
#		$subfield_data{marc_lib}=$tagslib->{$tag}->{$subfield}->{lib};
		...
		if ($tagslib->{$tag}->{$subfield}->{authorised_value}) {
			...
			if ($tagslib->{$tag}->{$subfield}->{'authorised_value'} eq "branches" ) {
				...
				$value=$input->param('branch');
				unless ($value) {$value=(split("_",(split(";",$cookie))[0]))[1];}
			...
			$subfield_data{marc_value}= CGI::scrolling_list(-name=>'field_value',											-values=> \@authorised_values,
						    -default=>"$value",														-labels => \%authorised_lib,  								                                    -size=>1,															-multiple=>0,												);
...
#		$subfield_data{marc_value}="<input type=\"text\" name=\"field_value\">";
	#	if ($prevtag eq $tag) {push(@loop_data, \%subfield_data);}
