...
# $Id: Reserves.pm,v 1.5.2.1 2003/12/17 22:22:05 slef Exp $
...
# C4::Reserves2. Pick one and stick with it.
sub CreateReserve {
  my ($env,$branch,$borrnum,$biblionumber,$constraint,$bibitems,$fee,$itemnumber) = @_;
  my $dbh = C4::Context->dbh;
  #$dbh->{RaiseError} = 1;
  #$dbh->{AutoCommit} = 0;
  my $const = lc substr($constraint,0,1);
    ...
    if ($fee > 0) {
      ...
      my $usth = $dbh->prepare("insert into accountlines
         (borrowernumber,accountno,date,amount,description,accounttype,amountoutstanding)
          values (?,?,now(),?,'Reserva','Res',?)");
      $usth->execute($borrnum,$nextacctno,$fee,$fee);
      $usth->finish;
    }
    my $sth = $dbh->prepare("insert into reserves (borrowernumber,biblionumber,reservedate,branchcode,constrainttype,itemnumber) values (?,?,?,?,?,?)");
    $sth->execute($borrnum,$biblionumber,$resdate,$branch,$const,$itemnumber);
