...
else if(jQuery.trim($('#codBarra').val())!=''){if((jQuery.trim($('#codBarra').val())).length<limite_caracteres){cumple_limite=false;}else{buscarPorCodigoBarra();}}
else if(jQuery.trim($('#isbn').val())!=''){if((jQuery.trim($('#isbn').val())).length<limite_caracteres){cumple_limite=false;}else{buscarPorISBN();}}
...
function buscarPorTema(){objAH=new AjaxHelper(updateInfoBusquedas);objAH.debug=true;objAH.showOverlay=true;objAH.url='/cgi-bin/koha/busquedas/busquedasDB.pl';objAH.tema=$('#tema').val();objAH.tipoAccion='BUSQUEDA_POR_TEMA';objAH.funcion='changePage';objAH.sendToServer();}
function buscarPorISBN(){objAH=new AjaxHelper(updateInfoBusquedas);objAH.showOverlay=true;objAH.debug=true;objAH.url='/cgi-bin/koha/busquedas/busquedasDB.pl';objAH.isbn=$('#isbn').val();objAH.tipoAccion='BUSQUEDA_POR_ISBN';objAH.only_available=($('#only_available').attr('checked'))?1:0;objAH.sendToServer();}
function buscarPorDiccionario(){objAH=new AjaxHelper(updateInfoBusquedas);objAH.debug=true;objAH.showOverlay=true;objAH.url='/cgi-bin/koha/busquedas/diccionario.pl';objAH.dictionary=$('#dictionary').val();objAH.only_available=($('#only_available').attr('checked'))?1:0;objAH.funcion='changePage';objAH.sendToServer();}
function buscarPorCodigoBarra(){objAH=new AjaxHelper(updateInfoBusquedas);objAH.debug=true;objAH.showOverlay=true;objAH.url='/cgi-bin/koha/busquedas/busquedasDB.pl';objAH.codBarra=$('#codBarra').val();objAH.only_available=($('#only_available').attr('checked'))?1:0;objAH.tipoAccion='BUSQUEDA_POR_BARCODE';objAH.sendToServer();}
...
keyword=$('#keyword-bar').val();keyword=keyword.replace(/\&/g,"AND");objAH=new AjaxHelper(updateBusquedaPorKeyword);objAH.showOverlay=true;objAH.debug=true;objAH.url='/cgi-bin/koha/busquedas/busquedasDB.pl';if(suggested){objAH.keyword=suggested;objAH.from_suggested=1;}else{objAH.keyword=keyword;}
...
function buscarEstante(){objAH=new AjaxHelper(updateInfoBusquedas);objAH.showOverlay=true;objAH.debug=true;objAH.url='/cgi-bin/koha/busquedas/estante.pl';objAH.viewShelfName=$('#estante').val();objAH.orden='title';objAH.only_available=($('#only_available').attr('checked'))?1:0;objAH.funcion='changePage';objAH.sendToServer();}
...
function verTema(idtema,tema){objAH=new AjaxHelper(updateInfoBusquedas);objAH.debug=true;objAH.showOverlay=true;objAH.url='/cgi-bin/koha/busquedas/busqueda.pl';objAH.idTema=idtema;objAH.tema=tema;objAH.funcion='changePage';objAH.sendToServer();}
function cambiarEstadoCampos(campos,clase){for(i=0;i<campos.length;i++){$('#'+campos[i]).attr('class',clase);}}
function buscarPorAutor(completo){objAH=new AjaxHelper(updateInfoBusquedas);objAH.showOverlay=true;objAH.url='/cgi-bin/koha/busquedas/busquedasDB.pl';objAH.debug=true;objAH.funcion='changePage';objAH.only_available=($('#only_available').attr('checked'))?1:0;objAH.completo=completo;objAH.tipoAccion="BUSQUEDA_POR_AUTOR";objAH.sendToServer();}
