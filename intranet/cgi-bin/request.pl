...
###MATIAS###
    ...
    foreach my $num (@barcodes) {
		...
		$barcode{'message'}=$num->{'itemlost'} == 1 ? "(Perdido)" :
	    $num->{'itemlost'} == 2 ? "(Muy atrasado)" : "";
...
my ($count,@reserves) = Findgroupreserve($dat->{'biblioitemnumber'},$bib);
my $totalcount = $count;
foreach my $res (@reserves) {
    if ($res->{'found'} eq 'W') {
        $count--;
...
my $options;
for (my $i=1; $i<=$num; $i++){
   if($i==$num){$options=$options."<option value=$i selected > $i </option>"; }
	else{ $options=$options."<option value=$i > $i </option>";}
}
 $abibitem{'options'}=$options;
...
##MATIAS
my ($cou,@rese) = Findgroupreserve($res->{'bbiblioitemnumber'},$bib);
                                                                                                                             
#priorityoptions building
my $options;
for (my $i=1; $i<=$cou; $i++){
   if($i==$res->{'priority'}){$options=$options."<option value=$i selected > $i </option>"; }
        else{ $options=$options."<option value=$i > $i </option>";}
}

 ($reserve{'itemtype'},$reserve{'description'}) = FindItemType($res->{'bbiblioitemnumber'});
 ($reserve{'volumeddesc'},$reserve{'volume'})   = FindVol($res->{'bbiblioitemnumber'});
$reserve{'bbiblioitemnumber'}= $res->{'bbiblioitemnumber'};
$reserve{'timestamp'}= $res->{'rtimestamp'};
