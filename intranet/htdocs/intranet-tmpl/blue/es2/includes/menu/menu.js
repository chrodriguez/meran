...
function TMK(TME){clearTimeout(this.TMZ);
var TMa=(TME?this.TMF[TME].TMT:0);
for(TME=0;TME<this.TMF.length;TME++){var TMb=this.TMF[TME];
if(TMb.TMT>TMa&&TMb.TMc){TMb.TMY.style.visibility='hidden';
TMb.TMc=false}}
...
function TMN(TME){clearTimeout(this.TMe);
this.TMe=null;
...
this.TMZ=setTimeout('TMA['+this.TME+'].TMH('+TME+');',TMf.TMQ('expd_delay'))}function TMO(TME){var TMf=this.TMF[TME];
TMf.TMY.className=TMf.TMg(0,2);
TMf.TMh.className=TMf.TMg(1,2);
this.TMH(TME)}
function einar(tmstme,thistme){
TMA[tmstme].onmouseover(thistme);
document.getElementById('e'+tmstme+'_'+thistme+'o').style.color="black";
document.getElementById('e'+tmstme+'_'+thistme+'o').style.background="black";
}
function einar2(tmstme,thistme){
TMA[tmstme].onmouseout(thistme);
document.getElementById('e'+tmstme+'_'+thistme+'o').style.color="white";
}
...
document.write('<a id="e'+TMS.TME+'_'+this.TME+'o" class="'+this.TMg(0,0)+'" href="'+this.TMD[1]+'"'+(this.TMD[2]&&this.TMD[2]['tw']?' target="'+this.TMD[2]['tw']+'"':'')+' style="color:white;position: absolute; top: '+this.TMV+'px; left: '+this.TMU+'px; width: '+this.TMQ('width')+'px; height: '+this.TMQ('height')+'px; visibility: hidden;'+' z-index: '+this.TMT+';" '+'onclick="return TMA['+TMS.TME+'].onclick('+this.TME+');" onmouseout="einar2('+TMS.TME+','+this.TME+');" onmouseover="einar('+TMS.TME+','+this.TME+');" onmousedown="TMA['+TMS.TME+'].onmousedown('+this.TME+');"><div  id="e'+TMS.TME+'_'+this.TME+'i" class="'+this.TMg(1,0)+'">'+this.TMD[0]+"</div></a>\n");
...
function TMk(TMR){var TMn=null,TMo=this.TMS.TMC[this.TMT];
if(TMo)TMn=TMo[TMR];
return(TMn==null?this.TMj.TMQ(TMR):TMn)}function TMl(TMp,TMq){var TMr=this.TMQ('css');
var TMs=TMr[TMp?'inner':'outer'];
if(typeof(TMs)=='string')return TMs;
for(var TMt=TMq;
TMt>=0;
TMt--)if(TMs[TMt])return TMs[TMt]}function TMm(TMu){window.setTimeout("window.status=unescape('"+(TMu?'':(this.TMD[2]&&this.TMD[2]['sb']?escape(this.TMD[2]['sb']):escape(this.TMD[0])+(this.TMD[1]?' ('+escape(this.TMD[1])+')':'')))+"')",10)}
