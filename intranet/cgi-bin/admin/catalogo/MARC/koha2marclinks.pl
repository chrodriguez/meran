...
my ($template, $borrowernumber, $cookie)
    = get_template_and_user({template_name => "admin/catalogo/MARC/koha2marclinks.tmpl",
			     query => $input,
			     type => "intranet",
			     authnotrequired => 0,
...
################## DEFAULT ##################################
	...
	$template->param(loop => \@loop_data,
							tablename => CGI::scrolling_list(-name=>'tablename',
																	-values=>['biblio','biblioitems','items','bibliosubject','bibliosubtitle','additionalauthors','publisher','isbns'],
																	-default=>$tablename,
																	-size=>1,
																	-multiple=>0
																	)
