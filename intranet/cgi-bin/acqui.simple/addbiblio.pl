...
sub build_authorized_values_list ($$$$$) {
    ...
    #---- Classification, Nivel bibliografico
    } elsif ($tagslib->{$tag}->{$subfield}->{authorised_value} eq "bibliolevel") {
	my $sth=$dbh->prepare("select code,description from bibliolevel");
	$sth->execute;
	push @authorised_values, ""
		unless ($tagslib->{$tag}->{$subfield}->{mandatory});
    ...
    #---- Pais
    } elsif ($tagslib->{$tag}->{$subfield}->{authorised_value} eq "countries") {
	my $sth=$dbh->prepare("select iso,printable_name from countries");
	$sth->execute;
	push @authorised_values, ""
		unless ($tagslib->{$tag}->{$subfield}->{mandatory});
   ...
   #-----Idioma
   } elsif ($tagslib->{$tag}->{$subfield}->{authorised_value} eq "languages") {
	my $sth=$dbh->prepare("select idLanguage,description from languages");
	$sth->execute;
	push @authorised_values, ""
		unless ($tagslib->{$tag}->{$subfield}->{mandatory});
	...
	$value="es";
   #-----Support
   } elsif ($tagslib->{$tag}->{$subfield}->{authorised_value} eq "supports") {
	my $sth=$dbh->prepare("select idSupport,description from supports");
	$sth->execute;
	push @authorised_values, ""
		unless ($tagslib->{$tag}->{$subfield}->{mandatory});
...
sub create_input () {
	...
	if ($tagslib->{$tag}->{$subfield}->{authorised_value}) {
		...
		$subfield_data{marc_value}="<input type=\"text\" name=\"field_value\"  size=47 DISABLE READONLY> <a href=\"javascript:Dopop('../thesaurus_popup.pl?category=$tagslib->{$tag}->{$subfield}->{thesaurus_category}&index=$i',$i)\">...</a>";
		...
		$subfield_data{marc_value}="<input type=\"text\" name=\"field_value\"  value=\"$value\" DISABLE READONLY size=47 OnFocus=\"javascript:Focus$function_name($i)\" OnBlur=\"javascript:Blur$function_name($i)\"> <a href=\"javascript:Clic$function_name($i)\">...</a> $javascript";
	} elsif  ($tag eq '') {
		$subfield_data{marc_value}="<input type=\"hidden\" name=\"field_value\" size=50>"; #"
	} else {
...
# now, loop again to add parameter subfield that are not in the MARC::Record
					...
					if ($#subfields_data >= 0) {
						...
						$tag_data{repeatable} = $tagslib->{$tag}->{repeatable}; #Agregado por Luciano
...
sub build_hidden_data () {
	...
	foreach my $subfield (keys %{$tagslib->{$tag}}) {
	    ...
	    next if ($subfield eq 'repeatable'); #Agregado por Luciano
...
#------------------------------------------------------------------------------------------------------------------------------
	my $current_tab= $input->param('current_tab') || 0;
